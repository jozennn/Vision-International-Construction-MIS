...
const TimelineGantt = ({ project, trackingData }) => {
    ...
    const exportExcel = async () => {
        ...
        [project.project_name, `${projectDuration}`, projectStart ?? '', projectEnd ?? '',
         new Date().toLocaleDateString('en-US'), ''].forEach((v, ci) => {
            const c = ws.getCell(r, ci + 1);
            c.value = v; c.font = { bold: true, size: 10, name: 'Arial' };
            c.alignment = ctr; c.border = allB;
        });
        ...
        ganttDates.forEach((d, di) => {
            const c = ws.getCell(r, 7 + di);
            c.value = fmtDate(d); c.font = { bold: true, size: 7, name: 'Arial', color: { argb: 'FFFFFFFF' } };
            c.fill = fill(NAVY); c.alignment = ctr; c.border = allB;
        });
        ...
        tasks.forEach(t => {
            if (t.type === 'group') {
                for (let ci = 1; ci <= 6 + ganttDates.length; ci++) {
                    ...
                    if (ci === 1) c.value = t.name;
                    c.font = { bold: true, size: 10, name: 'Arial' };
                    c.fill = fill(LGRAY); c.border = allB;
                ...
                [t.name, t.start, t.end, t.duration ?? '', t.unit, `${t.percent ?? 0}%`].forEach((v, ci) => {
                    const c = ws.getCell(r, ci + 1);
                    c.value = v; c.font = { name: 'Arial', size: 9 };
                    c.border = allB; c.alignment = ci === 0 ? lft : ctr;
                });
                ...
                ganttDates.forEach((d, di) => {
                    ...
                    c.value = inR ? 'TARGET' : '';
                    c.font  = { bold: true, size: 6, name: 'Arial', color: { argb: 'FFFFFFFF' } };
                    c.fill  = inR ? fill(RED) : { type: 'pattern', pattern: 'none' };
                    c.alignment = ctr; c.border = allB;
                });
                ...
                ganttDates.forEach((d, di) => {
                    ...
                    c.value = done ? 'ACTUAL' : '';
                    c.font  = { bold: true, size: 6, name: 'Arial', color: { argb: 'FFFFFFFF' } };
                    c.fill  = done ? fill(GREEN) : { type: 'pattern', pattern: 'none' };
                    c.alignment = ctr; c.border = allB;
                });
                ws.getRow(r).height = 16; r++;
            }
        ...
        r++;
        ws.getCell(r, 1).value = 'PROJECT DURATION';
        ws.getCell(r, 2).value = projectDuration;
        [1, 2].forEach(ci => {
        ...
        saveAs(new Blob([buf], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }),
            `${project.project_name}_Timeline.xlsx`);
    };
    ...
    return (
        <div className="tg-section">
            ...
            {/* ── Summary Cards (no installers count) ── */}
            ...
            <div className="tg-card">
                <div className="tg-card-header">
                    ...
                    <div className="tg-header-btns">
                        ...
                        <button className="tg-add-btn secondary" onClick={addGroup}>+ Add Section</button>
                    </div>
                </div>
                <div className="tg-scroll">
                    <table className="tg-table">
                        <thead>
                            <tr>
                                <th className="th-left" style={{ minWidth: 200 }}>TASK NAME</th>
                                <th>START DATE</th>
                                <th>END DATE</th>
                                <th>DURATION</th>
                                <th>UNIT</th>
                        ...
                        <tbody>
                            {tasks.length === 0 && (
                                <tr>
                                    <td colSpan={7} style={{ padding: '20px', textAlign: 'center', color: '#bbb', fontStyle: 'italic' }}>
                                        No tasks yet. Click "+ Add Task" to start building the timeline.
                                    </td>
                                </tr>
                            )}
                                ...
                                <tr key={t.id} className="tr-group">
                                    <td colSpan={6} className="td-left">
                                        <input type="text" value={t.name}
                                            ...
                                            className="tg-input group-input"
                                            placeholder="Section name..." />
                                    </td>
                                    <td>
                                        <button className="tg-remove-btn" onClick={() => removeTask(t.id)}>✕</button>
                                    </td>
                                </tr>
                            ) : (
                                <tr key={t.id}>
                                    <td className="td-left">
                                        <input type="text" value={t.name}
                                            onChange={e => updateTask(t.id, 'name', e.target.value)}
                                            className="tg-input" placeholder="Task description..." />
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <select value={t.unit}
                                            onChange={e => updateTask(t.id, 'unit', e.target.value)}
                                            className="tg-select">
                                            <option>DAYS</option>
                                            <option>HRS</option>
                                            <option>WKS</option>
                ...
                <div className="tg-gantt-card">
                    <div className="tg-gantt-header">
                        ...
                        <div className="tg-gantt-legend">
                            ...
                            <span className="tg-legend-actual">■ Actual (click to toggle)</span>
                        </div>
                    </div>
                    <div className="tg-gantt-scroll">
                        <table className="tg-gantt-table">
                            <thead>
                                <tr>
                                    <th className="tg-gantt-task-col">TASK NAME</th>
                                    {ganttDates.map((d, di) => (
                            ...
                            <tbody>
                                {tasks.map(t => t.type === 'group' ? (
                                    <tr key={t.id} className="tr-group">
                                        ...
                                        {ganttDates.map((_, di) => (
                                            <td key={di} className="tg-gantt-cell cell-group" />
                                        ))}
                                    </tr>
                                ) : (
                                    <React.Fragment key={t.id}>
                                        ...
                                        <tr>
                                            <td className="tg-gantt-name">
                                                <span className="tg-label-tag tg-label-target">T</span>
                                                {t.name}
                                            </td>
                                            {ganttDates.map((d, di) => {
                                                const ts = parseDate(t.start), te = parseDate(t.end);
                                                const inRange = ts && te && d >= ts && d <= te;
                                                return (
                                                    <td key={di} className={`tg-gantt-cell ${inRange ? 'cell-target' : ''}`} />
                                                );
                                            })}
                                        </tr>
                                        {/* Actual row */}
                                            ...
                                            {ganttDates.map((d, di) => {
                                                ...
                                                return (
                                                    <td key={di}
                                                        ...
                                                        title={`${dStr} — click to ${done ? 'unmark' : 'mark as done'}`} />
                                                );
                                            })}
                                        </tr>
                                    </React.Fragment>
                    ...
                    <p className="tg-gantt-hint">💡 Click any Actual row cell to mark it as complete</p>
                </div>
            )}

            {ganttDates.length === 0 && tasks.some(t => t.type === 'task') && (
                <div className="tg-empty">
                    Add start and end dates to tasks to generate the Gantt chart automatically.
                </div>
            )}
        </div>
    );
